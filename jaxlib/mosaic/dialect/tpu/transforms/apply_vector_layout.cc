...
LogicalResult tpu_matmul_rule(RewriteContext &ctx, Operation &op,
  ...
  const Value i32_mask_base = builder.create<arith::ConstantOp>(
  ...
  auto maskVregs = [&](xla::Array<Value> &vregs, int64_t padding_bottom,
    ...
    VLOG(0) << "[jevin debug] padding_bottom: " << padding_bottom;
    VLOG(0) << "[jevin debug] padding_right: " << padding_right;
    ...
    if (padding_bottom > 0) {
      ...
      // Eg. if padding_bottom = 3, packing = 2, and assume the vreg shape is
      ...
      // + packing).
      Value submask = builder.create<arith::ConstantOp>(
          op.getLoc(),
      ...
      submask = builder.create<arith::SelectOp>(
          getX32VmaskByPaddingEnd(0, padding_bottom + 1), i32_mask_base,
          submask);
      if (padding_bottom - 1 > 0) {
        submask = builder.create<arith::SelectOp>(
            getX32VmaskByPaddingEnd(0, padding_bottom - 1), submask,
            i32_zeros_vreg);
      }
      for (int64_t i = 0; i < vregs.dim(1); ++i) {
        ...
        if (sub_padding > 0) {
          i32_vreg = builder.create<arith::AndIOp>(i32_vreg, submask);
        } else {
  ...
  int64_t acc_lhs_compression = acc.getType().getElementTypeBitWidth() /
                                lhs.getType().getElementTypeBitWidth();
  ...
  xla::Array<Value> target_acc_vregs(
      {lhs_vregs.dim(0) * acc_lhs_compression, target_acc_col_vregs},
      acc_zeros_vreg);
