...
FailureOr<xla::Array<Value>> insertImplicitMinorDimension(
  ...
  // TODO(tlongeri): Would be nicer if we exposed a standalone function that
  // doesn't require an implicit dimension to be specified.
  ...
  new_vregs.Each([&](const absl::Span<const int64_t> dst_idx,
    ...
    if (!layout.offsets()[0].has_value() && new_3rd_minor_idx != 0) {
      ...
      *(dst_idx_local.end() - 3) = 0;
      *dst_vreg = new_vregs(dst_idx_local);
    } else if (!layout.offsets()[1].has_value() && new_2nd_minor_idx != 0) {
      SmallVector<int64_t> dst_idx_local(toArrayRef(dst_idx));
      *(dst_idx_local.end() - 2) = 0;
      *dst_vreg = new_vregs(dst_idx_local);
    } else {
      // Row, after offsets, of the represented value that will be partially
      // held by dst_vreg:
      const int64_t row_idx =
          layout.offsets()[0] ? new_3rd_minor_idx + *layout.offsets()[0] : 0;
      // Column, after offsets, of the represented value for the element that
      // will be in the first sublane of dst_vreg:
      ...
      src_idx.pop_back();
      *(src_idx.end() - 2) = row_idx / ctx.target_shape[0];
      *(src_idx.end() - 1) = col_idx / ctx.target_shape[1];
  ...
  auto getDstVregs = [&]() -> FailureOr<xla::Array<Value>> {
    if (no_op) {
      ...
      return dst_vregs_local;
               ...
               layout_in.hasNativeTiling(ctx.target_shape) &&
               layout_in.tiling() == layout_out.tiling() &&
               layout_in.offsets()[1].value_or(0) % ctx.target_shape[0] == 0 &&
               (layout_out.offsets()[0] == 0 ||
