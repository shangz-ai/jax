...
def _compile_to_executable(
  ...
  out_avals = tuple(
      jax.core.ShapedArray(aval.shape, aval.dtype) for aval in out_specs_leaves
  )
  out_shardings = tuple(aval.sharding for aval in out_specs_leaves)
  try:
    ...
    out_handlers = pxla.global_avals_to_results_handler(
        out_avals, out_shardings, committed=True
    ).handlers
